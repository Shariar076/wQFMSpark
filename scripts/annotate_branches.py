...
def read_tree(inputStreeFile):
    ...
    tree = dendropy.Tree.get_from_path(inputStreeFile, "newick", taxon_namespace=taxa,
                                       rooting="force-rooted")  ## rooting should be done
    return tree
...
def compute_tree_QSupport(tree, list_quartets):
    ...
    for nd in tree:
        if nd.bipartition.is_trivial() == False:
            ...
            support_values[nd.bipartition] = get_support_value(nd.bipartition, tree.taxon_namespace,
                                                               list_quartets) if nd.label is None else -1.0
